...
function startMazeGeneration() {
    let grid = createGrid(); // Assuming you have a function that creates the grid
    generateMaze(grid, 0, 0, gridWidth, gridHeight); // Trigger maze generation
    drawGrid(grid); // Assuming a function that renders the grid
}
...
function generateMaze(grid, xStart, yStart, width, height) {
    if (width <= 2 || height <= 2) return;  // Base case

    // Determine where to place the wall and opening
    let horizontal = Math.random() > 0.5;
    let wx = xStart + (horizontal ? 0 : Math.floor(Math.random() * (width - 1)));
    let wy = yStart + (horizontal ? Math.floor(Math.random() * (height - 1)) : 0);
    ...
    for (let i = 0; i < (horizontal ? width : height); i++) {
        let dx = wx + (horizontal ? i : 0);
        let dy = wy + (horizontal ? 0 : i);
        if (dx != px || dy != py) grid[dy][dx].isWall = true;
    }

    // Recursively divide sections
    if (horizontal) {
        generateMaze(grid, xStart, yStart, width, wy - yStart);
        generateMaze(grid, xStart, wy + 1, width, height - (wy - yStart + 1));
    } else {
        generateMaze(grid, xStart, yStart, wx - xStart, height);
        generateMaze(grid, wx + 1, yStart, width - (wx - xStart + 1), height);
    }
}
